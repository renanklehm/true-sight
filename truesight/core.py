...
class TrueSight:
    ...
    def get_model(
        ...
        for i in range(len(self.preprocessor.models)):
            ...
            x[i] = tf.keras.layers.Dense(hparams['hidden_size'], activation='relu', name=f"dense_output_{i}")(x[i])
        ...
        x[-1] = tfm.nlp.layers.TransformerEncoderBlock(hparams['num_heads'], hparams['hidden_size'], inner_activation='selu', name=f"transformer_encoder")(x[-1])
        #print(x[-1].shape)
        #x[-1] = tfm.nlp.layers.TransformerDecoderBlock(hparams['num_heads'], hparams['hidden_size'], intermediate_activation='selu', name=f"transformer_decoder")(x[-1])
        x[-1] = tf.keras.layers.Flatten(name=f"flatten_transformer")(x[-1])
        x[-1] = tf.keras.layers.Dense(hparams['hidden_size'], activation='relu', name=f"transformer_output")(x[-1])
        x_outputs.append(x[-1])

        y = tf.keras.layers.Concatenate(name="concatenate")(x_outputs)
        y = tf.keras.layers.Dense(hparams['hidden_size'], activation='relu', name="dense_output")(y)
        y = tf.keras.layers.Dropout(hparams['dropout_rate'], name="dropout_output")(y, training = True)
        y = tf.keras.layers.Dense(self.preprocessor.forecast_horizon, activation='relu', name="output_dense")(y)
