...
class TrueSight(tf.keras.Model):
    ...
    def __init__(
        ...
        if dataset.has_descriptors:
            ...
            self.descriptor_flatten = tf.keras.layers.Flatten(
                name='descriptor_flatten'
            )
            self.branches['descriptors'] = tf.keras.layers.Dense(
                context_size,
                activation='selu',
                name='descriptors'
            )
        ...
        for i in range(len(self.models)):
            self.branches[self.models[i]] = tf.keras.layers.Dense(
                context_size,
                activation='selu',
                name=f'{self.models[i]}'
            )

        self.branches['observed_data'] = tf.keras.layers.Dense(
            context_size,
            activation='selu',
            name='observed_data'
        )
        self.weighted_sum = ContextLayer(
            n_models=len(self.branches),
            name='weighted_sum'
        )
        self.num_feed_forward = NumericalFeedForward(
    ...
    def call(
        ...
        outputs = self.weighted_sum(outputs)                            # type: ignore
        outputs = self.num_feed_forward(outputs, training=True)         # type: ignore
        outputs = self.output_layer(outputs)                            # type: ignore
        return outputs
    ...
    def predict(
        self,
        input_tensor: tf.Tensor | np.ndarray | None = None,
        ...
        if input_tensor is None and dataset is None:
        ...
        if dataset is not None:
            ...
            prediction_input = _dataset.get_x()
        if input_tensor is not None:
            if not isinstance(input_tensor, (tf.Tensor, np.ndarray)):
                raise Exception("X must be either a tf.Tensor or np.ndarray.")
            prediction_input = input_tensor

        if dataset is None and input_tensor is None:
            raise Exception("Please provide either X or a dataset.")

        yhat = []
        for _ in range(n_repeats):
            yhat.append(super(TrueSight, self).predict(
                x=prediction_input,
    ...
    def plot_training_history(
        ...
        if not hasattr(self, 'history'):
            raise Exception('No history found. Please train the model first.')
